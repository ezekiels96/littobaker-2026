...
<div class="mx-auto max-w-3xl">
    {{-- Header --}}
    <div class="mb-6">
        <h1 class="font-gummy text-4xl font-extrabold lowercase text-lb-ink">edit gallery item</h1>
        <p class="mt-1 font-gummy text-sm font-bold lowercase text-lb-muted">
            update the image url, caption, and tags ✨
        </p>
    </div>
    ...
    <form action="{{ route('admin.gallery.update', $gallery) }}" method="POST"
        ...
        @php
            $existingTagsCsv = $gallery->tags?->pluck('name')->implode(', ') ?? '';
            $previewUrl = old('image_url', $gallery->image_url);
        @endphp
        ...
        <div class="relative flex flex-col gap-5">
            ...
            <div>
                ...
                <input
                    type="text"
                    name="tags_csv"
                    value="{{ old('tags_csv', $existingTagsCsv) }}"
                    class="mt-2 w-full rounded-2xl border border-black/10 bg-white/80 px-4 py-3 shadow-sm
                           focus:outline-none focus:ring-2 focus:ring-lb-pink/40"
